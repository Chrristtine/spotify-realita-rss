...
def get_releases(token):
    ...
    url = (
        ...
        "?include_groups=album,single&market=CZ&limit=10"
    )
    ...
    # Remove duplicate album IDs that can occur because of market/versions.
    unique = {}
...
def make_rss(releases):
    ...
    SubElement(channel, "description").text = "Nové skladby a releasy interpreta Realita na Spotify."
    ...
    for item in releases:
        ...
        SubElement(item_el, "guid", {"isPermaLink": "false"}).text = f"spotify:album:{item['id']}"
        SubElement(item_el, "pubDate").text = format_datetime(release_date)

        description = (
            f'<p><strong>{html.escape(title)}</strong></p>'
            f'<p>Nový release interpreta Realita na Spotify.</p>'
            f'<p><a href="{html.escape(spotify_url)}">Poslechnout na Spotify</a></p>'
        )
        if image:
            description = (
                f'<p><img src="{html.escape(image)}" alt="{html.escape(title)}" /></p>'
                + description
            )
